CREATE OR REPLACE TRIGGER TRG_AFT_INS_CLT_BP_TEXT
AFTER INSERT ON GL_BANK_PAYMENTS_DET
FOR EACH ROW
DECLARE
   -- local variables...
   v_TEXT_id      NUMBER(12);
...
BEGIN
  ...
  BEGIN
		SELECT ALERT_ACTIVE INTO V_ACTIVE FROM TEXT_ALERTS_TEMPLATE WHERE ALERT_ID = 'GBP';
	...
	IF V_ACTIVE = 1 THEN
	  ...
	  IF (v_post = 1 AND v_sms_service <> 0 AND v_phone IS NOT NULL) THEN
	     SELECT T.ALERT_BODY INTO v_text FROM TEXT_ALERTS_TEMPLATE T WHERE T.ALERT_ID = 'GBP';
	     ...
	     SELECT TEXT_ALERTS_SEQ.NEXTVAL, s.system_date
	       ...
	       VALUES(v_TEXT_id, NULL, v_phone, v_text, 'GBP', :NEW.GL_SL_CODE, v_sys_date, 'P', V_LOG_ID, 1);
	  END IF;
	END IF;  
END TRG_AFT_INS_CLT_BP_TEXT;
/
